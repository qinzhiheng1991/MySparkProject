...
/**
  ...
  *
  * Class used to solve an optimization problem using RMSProp Gradient Descent.
  *
  * @param gradient Gradient function to be used.
  * @param updater Updater to be used to update weights after every iteration.
  *
  */
class AdamGradientDescent(private var gradient: Gradient, private var updater: Updater)
  extends Optimizer with Logging {

  private var learning_rate: Double = 0.01
  private var momentum1: Double = 0.9
  private var momentum2: Double = 0.999
  ...
  def setMomentum1(momentum1: Double): this.type = {
    require(momentum1 > 0 && momentum1 < 1,
      s"momentum1 must be in range (0, 1) but got $momentum1")
    this.momentum1 = momentum1
    this
  }

  //gradient second order moment estimation
  def setMomentum2(momentum2: Double): this.type = {
    require(momentum2 > 0 && momentum2 < 1,
      s"momentum2 must be in range (0, 1) but got $momentum2")
    this.momentum2 = momentum2
    this
  }

  def setLearningRate(step: Double): this.type = {
    require(learning_rate > 0 && learning_rate <= 1.0,
      s"learning_rate must be in range (0, 1.0] but got $learning_rate")
  ...
  def optimize(data: RDD[(Vector, Array[Double])], initialWeights: Vector): Vector = {
    val (weights, _) = AdamGradientDescent.runMiniBatchAdamSGD(
      data,
      initialWeights,
      gradient,
      updater,
...
object AdamGradientDescent extends Logging {
  ...
  def runMiniBatchAdamSGD(data: RDD[(Vector, Array[Double])],
    ...
    val curCumSquareGradient = Vectors.zeros(n)
    val curCumGradient = Vectors.zeros(n)
    var converged = false
    var i = 1
    var time = 0
    var learning_rate_iter = learning_rate
    while (time < convergenceTime && i <= numIterations) {
      logInfo(s"runMiniBatchAdamSGD $i epoch starting...")
      if (decayRate > 0.0) {
      ...
      if (miniBatchSize > 0) {
        ...
        /**
          * compute regular gradient, gradientTemp = w - (w - regular_gradient) = regular_gradient
          */
        gradientTemp = weights.copy
        BLAS.axpy(-1.0, updater.adamCompute(weights,
                                            Vectors.zeros(n),
                                            Vectors.zeros(n),
                                            learning_rate_iter,
                                            epsilon,
                                            regParam)._1, gradientTemp)

        BLAS.axpy(1.0, gradientTemp, curGradientSum)

        val curCumSquareGradientValues = curCumSquareGradient.toDense.values
        val curCumGradientValues = curCumGradient.toDense.values

        var index = 0
        while (index < n) {
            curCumGradientValues(index) =
              (momentum1 * curCumGradientValues(index) + (1- momentum1) * curGradientSum(index)) / math.pow(momentum1, i)
            curCumSquareGradientValues(index) =
              (momentum2 * curCumSquareGradientValues(index) + (1 - momentum2) * math.pow(curGradientSum(index), 2)) / math.pow(momentum2, i)
          index += 1
        }

        val update = updater.adamCompute(weights, curCumGradient, curCumSquareGradient, learning_rate_iter, epsilon, regParam)
    ...
    logInfo("GradientDescent.runMiniBatchAdamSGD finished. Last 10 stochastic losses %s".format(
  ...
  /**
    * Alias of [[runMiniBatchAdamSGD]] with convergenceTol set to default value of 0.001.
    */
  def runMiniBatchAdamSGD(data: RDD[(Vector, Array[Double])],
                          initialWeights: Vector,
                          gradient: Gradient,
                          ...
                          classNum: Int): (Vector, Array[Double]) = {
    AdamGradientDescent.
      runMiniBatchAdamSGD(data, initialWeights, gradient, updater, momentum1, momentum2, learning_rate, epsilon,
  ...
  //Consider to introduce the last ten df/dx to estimate
