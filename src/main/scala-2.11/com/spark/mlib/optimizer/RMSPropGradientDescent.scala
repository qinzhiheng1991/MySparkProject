...
/**
  ...
  *
  */
class RMSPropGradientDescent(private var gradient: Gradient, private var updater: Updater)
  extends Optimizer with Logging {

  private var learning_rate: Double = 0.01
  private var momentum: Double = 0.8
  ...
  def optimize(data: RDD[(Vector, Array[Double])], initialWeights: Vector): Vector = {
    val (weights, _) = RMSPropGradientDescent.runMiniBatchRMSPropSGD(
      data,
      initialWeights,
      gradient,
      updater,
...
object RMSPropGradientDescent extends Logging {

  def runMiniBatchRMSPropSGD(data: RDD[(Vector, Array[Double])],
    ...
    while (time < convergenceTime && i <= numIterations) {
      logInfo(s"runMiniBatchRMSPropSGD $i epoch starting...")
      if (decayRate > 0.0) {
      ...
      if (miniBatchSize > 0) {
        ...
        while (index < n) {
          cumSquareGradientValues(index) = momentum * cumSquareGradientValues(index)
                                         + (1.0 - momentum) * math.pow(curGradientSum(index), 2)
          index += 1
        }
    ...
    logInfo("RMSPropGradientDescent.runMiniBatchRMSPropSGD finished. Last 10 stochastic losses %s".format(
  ...
  /**
    * Alias of [[runMiniBatchRMSPropSGD]] with convergenceTol set to default value of 0.001.
    */
  def runMiniBatchRMSPropSGD(data: RDD[(Vector, Array[Double])],
                             initialWeights: Vector,
                             gradient: Gradient,
    ...
    RMSPropGradientDescent.
      runMiniBatchRMSPropSGD(data, initialWeights, gradient, updater, momentum, learning_rate, epsilon,
