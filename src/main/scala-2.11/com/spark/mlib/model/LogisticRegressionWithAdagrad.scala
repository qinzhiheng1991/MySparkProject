...
import com.spark.mlib.optimizer.{AdagradGradientDescent, AdagradL1Updater, AdagradL2Updater, LogisticGradient}
...
object LogisticRegressionWithAdagrad {
  def train(data: RDD[MyLabeledPoint],
            ...
            classNum: Int = 2): LogisticRegressionWithAdagrad = {
    new LogisticRegressionWithAdagrad(numIterations, miniBatchFraction, stepSize, regParam, convergenceToVal, convergenceTime, classNum)
...
class LogisticRegressionWithAdagrad(private var numIterations: Int = 1000,
  ...
  def run(data: RDD[MyLabeledPoint], initialWeights: Vector): this.type = {
    ...
    val updater = updater_strategy.toUpperCase() match {
      case "L1" =>
        new AdagradL1Updater
      case "L2" =>
        new AdagradL2Updater
      case other =>
        throw new SparkException(s"wrong update strategy ${other.toUpperCase()}")
    }
    val optimizer = new AdagradGradientDescent(new LogisticGradient, updater)
