...
import com.spark.mlib.optimizer.{LogisticGradient, LogisticL1Updater, LogisticL2Updater, MomentumGradientDescent}
...
object LogisticRegressionWithMomentum {
  def train(data: RDD[MyLabeledPoint],
            ...
            classNum: Int = 2,
            momentum: Double = 0.8,
            isNesterov: Boolean = false): LogisticRegressionWithMomentum = {
    new LogisticRegressionWithMomentum(numIterations, miniBatchFraction, stepSize, regParam, convergenceToVal, convergenceTime, classNum)
      .setUpdaterStrategy(updater_strategy)
      .setInitStd(init_std)
      .setInitMean(init_mean)
      .setMomentum(momentum)
      .setNesterov(isNesterov)
      .run(data)
  }
}

class LogisticRegressionWithMomentum(private var numIterations: Int = 1000,
  ...
  def setMomentum(momentum: Double): this.type = {
    require(momentum >= 0 && momentum <= 1,
      s"momentum should be in range [0, 1], but momentum:$init_mean")
  ...
  def run(data: RDD[MyLabeledPoint], initialWeights: Vector): this.type = {
    ...
    val optimizer = new MomentumGradientDescent(new LogisticGradient, updater)
