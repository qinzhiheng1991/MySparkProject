...
import com.spark.mlib.optimizer.{LogisticGradient, LogisticL1Updater, LogisticL2Updater, StochasticGradientDescent}
...
/**
  ...
  * scattered continuous feature:
  * 1. buckets: based on equidistant or based on equifrequent
  * 2. uncertainty: decrease the entropy of the system to map features into scattered ones
  */

object LogisticRegressionWithGD {
  def train(data: RDD[MyLabeledPoint],
            ...
            classNum: Int = 2): LogisticRegressionWithGD = {
    new LogisticRegressionWithGD(numIterations, miniBatchFraction, stepSize, regParam, convergenceToVal, convergenceTime, classNum)
...
class LogisticRegressionWithGD(private var numIterations: Int = 1000,
  ...
  def run(data: RDD[MyLabeledPoint], initialWeights: Vector): this.type = {
    ...
    val optimizer = new StochasticGradientDescent(new LogisticGradient, updater)
  ...
  def save(ss: SparkSession, path: String = "LogisticRegressionWithGD"): Unit = {
