...
import com.spark.mlib.optimizer.{AdamGradientDescent, AdamL1Updater, AdamL2Updater, LogisticGradient}
...
object LogisticRegressionWithAdam {
  def train(data: RDD[MyLabeledPoint],
            ...
            momentum1: Double = 0.9,
            momentum2: Double = 0.99,
            classNum: Int = 2): LogisticRegressionWithAdam = {
    new LogisticRegressionWithAdam(numIterations, miniBatchFraction, stepSize, regParam, convergenceToVal, convergenceTime, classNum)
      ...
      .setMomentum1(momentum1)
      .setMomentum2(momentum2)
      .run(data)
  }
}

class LogisticRegressionWithAdam(private var numIterations: Int = 1000,
  ...
  private var momentum1 = 0.9
  private var momentum2 = 0.99
  ...
  def setDecayRate(decayRate: Double): this.type = {
    require(decayRate > 0 && decayRate < 1,
      s"decayRate should be in range (0, 1), but decayRate:$decayRate")
  ...
  def run(data: RDD[MyLabeledPoint], initialWeights: Vector): this.type = {
    ...
    val optimizer = new AdamGradientDescent(new LogisticGradient, updater)
